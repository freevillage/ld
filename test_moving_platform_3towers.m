...
sourcePosition = [ ...
    ...
    -50, 800, 0 ; ...
    0, 0, 0 ];
freqMin = [300, 305, 310] * 10^6; % minimum possible frequency
freqMax = [300, 305, 310] * 10^6; % maximum possible frequency
...
source = cell( totalSources, 1 );
for iSource = 1 : totalSources
    source{iSource} = PolychromaticSource( ...
...
GetArrayPosition = @(t) initialArrayPosition + arrayHorzSpeed * [ t; sin(t)/100; cos(t)/100 ];
...
fastSamplingPeriod = 1/8 / max( freqCentral );
...
fastRecordingTimeShort = 1 * fastSamplingPeriod;
...
if isGraphicsEnabled
    ...
    for iSource = 1 : totalSources
        ...
        subplot( 1, totalSources, iSource )
        plot( ...
            1e9 * fastTimeLong(1:100,1), abs( sampleTrace(1:100) ), ...
            1e9 * fastTimeLong(1:100,1), abs( sampleCleanTrace(1:100) ) ...
            )
        xlabel( 'Fast time [ns]' )
        title( sprintf( 'Source #%d', iSource ) )
    ...
    superTitle = sprintf( 'Amplitudes of sample noisy traces (SNR=%.0fdB)', ...
        noiseSnr );
    suptitle( superTitle )
end
...
for iSource = 1 : totalSources
    anglesTrue(:,:,:,iSource) = MultidimensionalArrayFun( ...
        @(time) TrueAngles( movingUraFine, source(iSource), time ), ...
        fastTimeShort ...
        );
end
...
for is = 1 : totalSources
    sourcePositionEstimated(:,is) = Doa2Pos( ...
        ...
        anglesEstimated(1,:,is), anglesEstimated(2,:,is) ...
        );
end

sourcePositionEstimationError = ColumnNorm( sourcePositionEstimated - [source.position] );

if isGraphicsEnabled
    figure, hold on
    Plot3dData(sourcePositionEstimated, 'rp', 'MarkerFaceColor', 'r' );
    Plot3dData([source.position], 'k.', 'MarkerFaceColor', 'k' );
    
    for is = 1 : totalSources
        text( source(is).position(1) + 20 , source(is).position(2), source(is).position(3), ...
...
for iSource = 1 : totalSources
    recordedDataCoarseSource = recordedDataCoarse{iSource};
    ...
    parfor kSlow = 1 : totalSlowTimes        
        freqsEstimated(:,:,:,kSlow,iSource) = DiscreteFrequencySpectrumHertzUra( ...
            recordedDataCoarseSource(:,:,:,kSlow), ...
...
% 
ix = 1; jy = 1;
profilesTrue = repmat( freqSource, [ totalSlowTimes, 1 ] );
profilesEstimated = squeeze( freqsEstimated(ix,jy,1,:,:) );
    ...
    for jy = 1 : movingUraCoarse.array.totalCols
        for kSlow = 1 : totalSlowTimes
            for iSource = 1 : totalSources
                ...
                speedEstimated(ix,jy,kSlow,iSource) = VelocityFromDoppletShift( ...
                    theseFreqsSource, ...
                    theseFreqsEstimated );
            end
        end
...
profilesEstimated = squeeze( speedEstimated(ix,jy,:,:) );
...
for iSource = 1 : totalSources
    ...
    for kSlow = 1 : totalSlowTimes
        theseAnglesEstimated = num2cell( anglesEstimated(:,kSlow,iSource) );
        ...
        if kSlow > 1 && kSlow < totalSlowTimes
            ...
            thisbvel(kSlow) = mean( ToColumn( speedEstimated(:,:,kSlow,iSource) ) );
        end
    end
...
if isGraphicsEnabled
    ...
    suptitle( 'Geolocation errors using DOA alone or together with FDOA' )
end
...
save( tempname( './mat40' ), ...
    ...
    'sourcePositionEstimated', 'sourcePosition' );