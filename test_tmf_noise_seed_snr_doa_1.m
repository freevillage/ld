...
dataSNRDOA = dataSNR + 10 * log10( fastTimeSamplingFrequency ...
    * repmat( fastTimeEnd', [1 totalSigmas totalSeeds] ) );
allErrors = ToColumn(meanLocationErrors);
[allSnrs, indSort] = sort( ToColumn( dataSNRDOA(:,:,1) ) );
allErrors = allErrors(indSort);

semilogy( allSnrs, allErrors, '-o', 'MarkerFaceColor', 'auto' )
grid on
xlabel( 'SNR_{DOA} [dB]' )
