...
Fmax = 300 * 10^6; % maximum possible frequency
totalSources = 1; % number of monochromatic components in a source
c = LightSpeed;
Fsrc = sort( RandomUniform( Fmin, Fmax, [1 totalSources] ) ); % source frequencies
phiSrc = RandomUniform( 0, 2*pi, [1 totalSources] ); % source phases

Asrc = ones( [1 totalSources] ); % amplitudes
