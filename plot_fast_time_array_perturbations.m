...
sigmaArrayPerturbation = linspace( 1, 50, 20 );
...
for oPerturbation = 1 : totalArrayPerturbations
    ...
    for iPeriod = 1 : totalPeriods
        if isscalar( thisArraySpeed ) || thisArraySpeed(2) == 0
            filename = sprintf( ...
                'test_doa_results_%.0f_%.2f_%.2f_%.2f_%.2f_%.2f_%.2f.mat', ...
            ...
            filename = sprintf( ...
                'test_doa_results_%.0f_%.2f_%.2f_%.2f_%.2f_%.2f-%.2f_%.2f.mat', ...
...
orient landscape
arrayPerturbationLabels = arrayfun( @(f) [num2str(round(f)), '%'], sigmaArrayPerturbation, ...
    'UniformOutput', false );
...
figure( 'Visible', 'On' )
semilogy( sigmaArrayPerturbation, errorOptimal(:,1), '-o', 'LineWidth', 2, 'MarkerFaceColor', 'auto' )
