function test_doa_methods_all( fastRecordingTimeInPeriods, initialAltitude, clockShift, aspectAngle, sigmaNoise, arraySpeed )

if nargin < 6, arraySpeed = 100000; end

close all 
saveDirectory = '/scratch/poliann/mat_all_new';
isWaitbarShown = false;

saveFilename = sprintf( 'test_doa_results_%.0f_%.2f_%.2f_%.2f_%.2f_%.2f.mat', ...
    fastRecordingTimeInPeriods, initialAltitude, clockShift, aspectAngle, log10(sigmaNoise), arraySpeed );
saveFilename = fullfile( saveDirectory, saveFilename );
if exist( saveFilename, 'file' ), return; end
...
GetArrayPosition = @(t) initialArrayPosition + arraySpeed * t * flightDirection;
