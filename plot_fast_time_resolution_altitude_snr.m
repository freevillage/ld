    ...
    for iPeriod = 1 : totalPeriods
        if isscalar( thisArraySpeed ) || thisArraySpeed(2) == 0
            filename = sprintf( ...
                ...
                numberPeriods(iPeriod), initialAltitude(jAltitude), clockShift, aspectAngles, log10(sigmaNoise*(3000/initialAltitude(jAltitude))), thisArraySpeed(1), mean( arrayPerturbations.dx ) );
        else
            filename = sprintf( ...
                'test_doa_results_%.0f_%.2f_%.2f_%.2f_%.2f_%.2f-%.2f_%.8f.mat', ...
                numberPeriods(iPeriod), initialAltitude(jAltitude), clockShift, aspectAngles, log10(sigmaNoise*(3000/initialAltitude(jAltitude))), thisArraySpeed(1), thisArraySpeed(2), mean( arrayPerturbations.dx ) );
        end
...
totalError =  squeeze( max( angleEstimationErrorNorm(:,:,:,1), [], 1 ) );
plotTotalError = loglog( ...
    fastTimePeriod*numberPeriods, totalError, ...
    '-', ...
    'LineWidth', 2, ...
    'MarkerFaceColor', 'auto' );
...
figure( 'Visible', 'On' )
loglog( initialAltitude, min(totalError,[],2), '-o', 'LineWidth', 2, 'MarkerFaceColor', 'auto' )
xlabel( 'Range [m]' )
