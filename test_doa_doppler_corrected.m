function test_doa_doppler_corrected( fastRecordingTimeInPeriods, initialAltitude, clockShift, aspectAngle, sigmaNoise, arraySpeed )
...
if arrayAcceleration == 0
    saveFilename = sprintf( ...
        'test_doa_doppler_corrected_results_%.0f_%.2f_%.2f_%.2f_%.2f_%.2f.mat', ...
        fastRecordingTimeInPeriods, initialAltitude, clockShift, aspectAngle, log10(sigmaNoise), arrayInitialSpeed );
else
    saveFilename = sprintf( ...
        'test_doa_doppler_corrected_results_%.0f_%.2f_%.2f_%.2f_%.2f_%.2f-%.2f.mat', ...
        fastRecordingTimeInPeriods, initialAltitude, clockShift, aspectAngle, log10(sigmaNoise), arrayInitialSpeed, arrayAcceleration );
end
...
%     thisMethodName = thisMethod{2};
    ...
    for kSlow = 1 : totalSlowTimes
        freqCentralCorrected = freqCentral * ( 1 - cos(aspectAngle) * arrayInitialSpeed/c );
        %freqCentralCorrected = freqCentral;
        anglesEstimated(:,kSlow,iMethod) = DirectionsOfArrivalUraSingleRowCol( ...
            FastTimeGather( recordedData, kSlow ), ...
            freqCentralCorrected + clockShift, ...
