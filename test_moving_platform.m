...
for it = 1 : totalTimes
    arrayPosition(it,:) = initialArrayPosition + velocity * motionDirection * time(it);
    alpha(it) = 0; % Rotation around x-axis
    beta(it) = 0; % Rotation around y-axis
    gamma(it) = 0; % Rotation around z-axis
end
...
for kt = 1 : totalTimes
    anglesEstimated(kt,:) = MusicDoaUra ( ...
...
figure
subplot( 2, 2, 1 )
plot( time, radtodeg( anglesTrue(:,1) ), time, radtodeg( anglesEstimated(:,1) ) )
xlabel( 'Time [s]' ), ylabel( 'Angle [deg]' )
title( 'Direction of arrival' )
legend( 'True', 'Estimated' )

subplot( 2, 2, 2 )
plot( time, radtodeg( anglesTrue(:,2) ), time, radtodeg( anglesEstimated(:,2) ) )
xlabel( 'Time [s]' ), ylabel( 'Angle [deg]' )
title( 'Direction of arrival' )
legend( 'True', 'Estimated' )

subplot( 2, 2, 3 )
plot( time, RelativeError( anglesTrue(:,1), anglesEstimated(:,1) ) * 100 )
xlabel( 'Time [s]' ), ylabel( 'Relative error [percent]' )
title( 'Estimation error' )

subplot( 2, 2, 4 )
plot( time, RelativeError( anglesTrue(:,2) + eps, anglesEstimated(:,2) ) * 100 )
xlabel( 'Time [s]' ), ylabel( 'Relative error [percent]' )
title( 'Estimation error' )
