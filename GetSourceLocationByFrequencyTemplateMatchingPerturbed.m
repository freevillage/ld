function output = GetSourceLocationByFrequencyTemplateMatchingPerturbed( varargin )
...
defaultArrayPerturbation = struct( 'i', 1, 'j', 1, 'dx', 0, 'dy', 0 );

addParameter( input, 'ArrayPerturbations', defaultArrayPerturbation, @isstruct )
...
arrayPerturbations = input.Results.ArrayPerturbations;
...
assumedZ = 0.00;
CostFunction = @(y) 1 -TemplateMatchTest( [y;assumedZ] );
...
ExtendedCostFunction = @(y2df) -ExtendedTemplateMatchTest( [y2df(1:2) ; assumedZ], y2df(3) );

%ExtendedCostFunction = @(y2dfphi)  -ExtendedTemplateMatchTest( [y2dfphi(1:2) ; 0] , y2dfphi(3), y2dfphi(4) );
extendedDeltaBounds = [ 1 ; 1 ; 1000  ];
extendedGuess = [ yEstimated(1:2); input.Results.SourceFrequency ];
...
[yfEstimated,costMinExtended] = fminunc( extendedTemplateMatchingProblem );
...
output = struct( ...
    'sourceLocationCoarse', [ yEstimated ; 0 ], ...
    'sourceLocation', [yfEstimated(1:2);0], ...
    'sourceFrequency', yfEstimated(3), ...
    'dataSNR', dataSNR ...
    );
