...
arrayLengthX = 1;
arrayLengthY = 1;
totalAntennasXFine = 11;
totalAntennasYFine = 13;
...
GetArrayPosition = @(t) bsxfun( @plus, initialArrayPosition, arrayHorzSpeed * [ t; sin(t)/0.3; cos(t)/0.3 ] );
...
fastRecordingTimeLong = 1000 * fastSamplingPeriod;
...
fastRecordingTimeShort = 400 * fastSamplingPeriod;
...
%                 downsampledDataCoarse{iSource}(ix,jy,:,ks)  = AddWhiteGaussianNoise( downsampledDataCoarse{iSource}(ix,jy,:,ks), noiseSnr, 'measured' );
%                 downsampledDataCoarse2{iSource}(ix,jy,:,ks) = AddWhiteGaussianNoise( downsampledDataCoarse2{iSource}(ix,jy,:,ks), noiseSnr, 'measured' ); 
            end
        end
...
%                 downsampledDataFine{iSource}(ix,jy,:,ks) = AddWhiteGaussianNoise( downsampledDataFine{iSource}(ix,jy,:,ks), noiseSnr, 'measured' );
%                 downsampledDataFine2{iSource}(ix,jy,:,ks) = AddWhiteGaussianNoise( downsampledDataFine2{iSource}(ix,jy,:,ks), noiseSnr, 'measured' );
            end
        end
...
%     figure
%     for iSource = 1 : totalSources
%         sampleTrace = squeeze( downsampledDataCoarse{iSource}(1,1,:,1) );
%         subplot( totalSources, 1, iSource )
%         plot( ...
...
if isGraphicsEnabled
    figure
    for iSource = 1 : totalSources
        sampleTrace = squeeze( recordedDataFine{iSource}(1,1,:,1) );
        sampleCleanTrace = squeeze( cleanDataFine{iSource}(1,1,:,1) );
        subplot( totalSources, 1, iSource )
        plot( ...
            1e6 * fastTimeShort(:,1), sampleTrace(:), ...
            1e6 * fastTimeShort(:,1), sampleCleanTrace(:) ...
            )
        xlabel( 'Fast time [\mus]' )
        title( sprintf( 'Source #%d', iSource ) )
        xlim( [0 1 ] )
    ...
    figure
    for iSource = 1 : totalSources
        sampleTrace = squeeze( downsampledDataFine{iSource}(1,1,:,1) );
        subplot( totalSources, 1, iSource )
        plot( ...
            1e6 * downsampledTimeShort(:,1), real(sampleTrace(:)), ...
            1e6 * downsampledTimeShort(:,1), imag(sampleTrace(:)) ...
            )
        xlabel( 'Fast time [\mus]' )
        title( sprintf( 'Source #%d', iSource ) )
        xlim( [0 1 ] )
...
methodUsed = 'TotalLeastSquaresProny';
...
anglesEstimatedDownsampled  = nan( 2, totalSlowTimes, totalSources );
anglesEstimated2Downsampled = nan( 2, totalSlowTimes, totalSources );
...
for iSource = 1 : totalSources
    ...
    for ks = 1 : totalSlowTimes
        %AverageOverFastTimes = @(data) squeeze( mean( data, 3 ) );
        WindowFastTimes = @(data) data(:,:,1:min([size(data,3),40]));

        
        recordedDataIsourceTrimmed = WindowFastTimes( recordedDataIsource(:,:,:,ks) );
        recordedDataIsourceTrimmed2 = WindowFastTimes( recordedDataIsource2(:,:,:,ks) );
        
        downsampledDataIsourceTrimmed = WindowFastTimes( downsampledDataIsource(:,:,:,ks) );
        downsampledDataIsource2Trimmed = WindowFastTimes( downsampledDataIsource2(:,:,:,ks) );
        
        anglesEstimated(:,ks,iSource) = DirectionsOfArrivalUra( ...
            recordedDataIsourceTrimmed, ...
            freqCentral(iSource), ...
            [arraySpacingXFine arraySpacingYFine], ...
            'Method', methodUsed, ...
            'TotalFrequencies', totalSourceComponents );
        
        anglesEstimatedDownsampled(:,ks,iSource) = DirectionsOfArrivalUra( ...
            downsampledDataIsourceTrimmed, ...
            freqCentral(iSource), ...
            [arraySpacingXFine arraySpacingYFine], ...
            'Method', methodUsed, ...
            'TotalFrequencies', totalSourceComponents );
        ...
        anglesEstimated2Downsampled(:,ks,iSource) = DirectionsOfArrivalUra( ...
            downsampledDataIsource2Trimmed, ...
            freqCentral(iSource)-freqCarrier, ...
            [arraySpacingXFine arraySpacingYFine], ...
            'Method', methodUsed, ...
            'TotalFrequencies', totalSourceComponents );
...
if isGraphicsEnabled
    ...
    subplot( 2, 2, 3 )
    plot( ToColumn(slowTime), radtodeg( minus( anglesTrueX, squeeze( anglesEstimatedDownsampled(1,:,:) ) ) ) )
    xlabel( 'Time [s]' ), ylabel( 'Error [deg]' )
    LegendWithLaTeX( arrayfun( @(is) [ '$\phi_{x,', num2str(is), '}-\widehat{\phi_{x,', num2str(is), '}}$'], 1:3, 'UniformOutput', false ) );
    title( 'Estimation errors' )
    
    subplot( 2, 2, 4 )
    plot( ToColumn(slowTime), radtodeg( minus( anglesTrueY, squeeze( anglesEstimatedDownsampled(2,:,:) ) ) ) )
    xlabel( 'Time [s]' ), ylabel( 'Error [deg]' )
    LegendWithLaTeX( arrayfun( @(is) [ '$\phi_{y,', num2str(is), '}-\widehat{\phi_{y,', num2str(is), '}}$'], 1:3, 'UniformOutput', false ) );
    title( 'Estimation errors' )
end
